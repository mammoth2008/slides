...
- 推荐命令：
  `python3 .claude/skills/slides-workflow/scripts/generate_html.py --course gai /Users/Freeman/Documents/products/courses/library/texts/gai-slide-drafts/slide-drafts/gai-3-2.md`
...
5. **干跑确认** — `python3 .claude/skills/build-layout/auto_layout.py {name}.yaml --dry-run`，打印坐标预览
6. **正式运行** — 确认无误后运行 `python3 .claude/skills/build-layout/auto_layout.py {name}.yaml`，生成布局
...
处理 impress.js 布局的校验和自动生成。完整文档见 `.claude/skills/build-layout/SKILL.md`。
...
使用方式：`python3 .claude/skills/slides-workflow/scripts/generate_html.py --course gai <source.md>`
使用方式：`python3 .claude/skills/slides-workflow/chinese_fix.py {filename.md}`
...
```
第1步：Claude 读课件 md，起草 mindmap-md 初稿
        → 读课程资料库中对应的课件 md 全文
        → 识别核心主题、知识模块、子主题
        → 按 #/##/###/- 层级重组，写出初稿
        → 告知用户"请审阅 mindmap-md 初稿"，等待确认
  ↓
第2步：用户审阅，Claude 按反馈修改，直到用户同意
  ↓
第3步：运行脚本生成 HTML
  python3 .claude/skills/slides-workflow/scripts/mindmap_gen.py \
